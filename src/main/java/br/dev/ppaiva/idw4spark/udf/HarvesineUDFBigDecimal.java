...
public class HarvesineUDFBigDecimal
		implements UDF4<BigDecimal, BigDecimal, BigDecimal, BigDecimal, Double>, Serializable {
	...
	public Double call(BigDecimal lon1, BigDecimal lat1, BigDecimal lon2, BigDecimal lat2) throws Exception {
		...
		double a = Math.sin(latDistance / 2) * Math.sin(latDistance / 2) + Math.cos(Math.toRadians(lat1Double))
				* Math.cos(Math.toRadians(lat2Double)) * Math.sin(lonDistance / 2) * Math.sin(lonDistance / 2);
